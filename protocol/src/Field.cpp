...
namespace Protocol {
    ...
    FieldPacket::FieldPacket(std::string name, uint32_t type, std::string table, 
        std::string orgTable, std::string dataBase, std::string orgName, uint32_t columnLength,
        uint32_t charset, uint32_t decimals, uint32_t flags) : impl_(new Impl()) {
        impl_->name = name;
        impl_->type = type;
        impl_->table = table;
    ...
    bool FieldPacket::UnPack(std::vector< uint8_t >& packet) {
        ...
        // convert mysql flag ...
        impl_->type = static_cast< uint32_t >(MySQLToType(static_cast< int64_t >(t), static_cast< int64_t >(impl_->flags)));
    ...
    std::vector< uint8_t > FieldPacket::Pack() {
        ...
        // flags
        WriteU16(outputPacket, static_cast< uint16_t >(TYPE_TO_MYSQL[impl_->type].second));
        // Decimals
    ...
    std::string FieldPacket::ToString() {
        return "name: " + impl_->name + ", type: " + std::to_string(impl_->type) + ", table: " +
            impl_->table + ", orgTable: " + impl_->orgTable + ", dataBase: " + impl_->dataBase +
            ", orgName: " + impl_->orgName + ", columnLength: " + std::to_string(impl_->columnLength) +
            ", charset: " + std::to_string(impl_->charset) + ", decimals: " + std::to_string(impl_->decimals) +
