...
namespace Protocol {
    ...
    EofPacket::EofPacket(uint16_t warnFlag, uint16_t statusFlag) : impl_(new Impl()) {
    ...
    bool EofPacket::UnPack(std::vector< uint8_t >& packet) {
        ...
        // warning
        impl_->warnings = static_cast< uint16_t >(*seek) |
                          static_cast< uint16_t >(*(seek + 1) << 8);
        seek += 2;
        // status
        impl_->statusFlags = static_cast< uint16_t >(*seek) | 
                             static_cast< uint16_t >(*(seek + 1) << 8);
        return true;
    }
     ...
     * 状态标志
     */
    uint16_t EofPacket::GetStatusFlags(){
        return impl_->statusFlags;
    }
