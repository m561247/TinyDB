...
namespace Protocol {
    ...
    GreetingPacket::~GreetingPacket() noexcept {
        // auto 
    }

    GreetingPacket::GreetingPacket(uint32_t connectionID, std::string serverVersion) : impl_(new Impl()) {
    ...
    bool GreetingPacket::UnPack(std::vector< uint8_t >& packet) {
        ...
        seek ++;
        // serverVersion
        std::vector< uint8_t >::iterator serverVersionEnd = std::find(seek, packet.end(), 0x00);
        ...
        seek ++;
        // capLower
        capLower = static_cast< uint16_t >(*seek) |
                   static_cast< uint16_t >(*(seek + 1) << 8);
        seek += 2;
        // character set
        impl_->charset = *seek;
        seek ++;
        // status flags
        impl_->status = static_cast< uint16_t >(*seek) |
                        static_cast< uint16_t >(*(seek + 1) << 8);
        seek += 2;
        // capUpper
        capUpper = static_cast< uint16_t >(*seek) |
                   static_cast< uint16_t >(*(seek + 1) << 8);
        seek += 2;
        impl_->capability = static_cast< uint32_t >(capUpper) << 16 | static_cast<uint32_t>(capLower);
        ...
        // 0 filter
        seek ++;
        std::vector< uint8_t >::iterator authPluginNameEnd = std::find(seek, packet.end(), 0x00);
    ...
    std::string GreetingPacket::GetServerVersion() {
        return impl_->serverVersion;
    }

    uint32_t GreetingPacket::GetConnectionID() {
        return impl_->connectionID;
    }
    ...
    uint16_t GreetingPacket::GetServerStatus() {
        return impl_->status;
    }
