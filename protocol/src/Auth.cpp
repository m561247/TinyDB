...
namespace Protocol {
    ...
    AuthPacket::~AuthPacket() noexcept {}
    ...
    bool AuthPacket::UnPack(std::vector< uint8_t >& packet) {
        ...
        if ((impl_->clientFlags & CLIENT_CONNECT_WITH_DB) > 0) {
            //init db name
            std::vector<uint8_t>::iterator dbNameEnd = std::find(seek, packet.end(), 0x00);
        ...
        if ((impl_->clientFlags & CLIENT_PLUGIN_AUTH) > 0) {
            std::vector<uint8_t>::iterator pluginNameEnd = std::find(seek, packet.end(), 0x00);
    ...
    std::string AuthPacket::GetDatabaseName() {
        return impl_->database;
    }

    std::string AuthPacket::GetPluginName() {
        return impl_->pluginName;
    }

    std::string AuthPacket::GetUserName() {
        return impl_->user;
    }
